...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate plan-delta JSON against schema.")
    parser.add_argument("paths", nargs="+", help="JSON files, directories, or globs")
    parser.add_argument("--schema", default=str(Path(__file__).resolve().parents[1] / "assets" / "schemas" / "plan-delta.schema.json"))
